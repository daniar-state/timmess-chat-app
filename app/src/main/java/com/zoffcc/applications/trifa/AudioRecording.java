    ...
    public void run()
    {
        ...
        try
        {
            ...
            if (PREF__use_native_audio_play)
            {
                ...
                int audio_buffer_mult = 1;

                int want_buf_size_in_bytes = (int) ((48000.0f / 1000.0f) * milliseconds_audio_samples_max * 2 * 2 * audio_buffer_mult);
                Log.i(TAG, "want_buf_size_in_bytes(1)=" + want_buf_size_in_bytes);

                _recBuffer = ByteBuffer.allocateDirect(
                    (int) ((48000.0f / 1000.0f) * milliseconds_audio_samples_max * 2 * 2 * audio_buffer_mult)); // Max 120 ms @ 48 kHz
